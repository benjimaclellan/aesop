...
class PulseRepetition(Evaluator):
    ...
    def __init__(self, propagator, **kwargs):
        ...
        self.scale_array = (np.fft.fftshift(
            np.linspace(0, len(self.target_rf) - 1, len(self.target_rf))) / propagator.n_samples).reshape(
    ...
    def evaluate_graph(self, graph, propagator):
        ...
        overlap = self.cosine_waveform_temporal_overlap(state, propagator)
    ...
    def cosine_waveform_temporal_overlap(self, state, propagator):
        generated = power_(state)
        shifted = self.shift_function(generated, propagator)
        # shifted = shifted/np.max(shifted)
        # similarity = -self.similarity_cosine(shifted, self.target)
        similarity = self.similarity_l2_norm(shifted, self.target)
        return similarity
