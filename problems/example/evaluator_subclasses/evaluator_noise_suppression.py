...
class NoiseSuppression(Evaluator):
    """  """

    def __init__(self, propagator, true_signal=None, evaluation_node='sink', **kwargs):
        super().__init__(**kwargs)

        if true_signal is None:
            raise RuntimeError('Need to specify the true signal')
        self.true_signal = true_signal  # true signal from the NoisySignal class (i.e. one sinc pulse for now)
        self.true_signal_power = power_(true_signal)
    ...
    def evaluate_graph(self, graph, propagator):
        ...
        # shifted = self.shift_function
        # score = self.similarity_l1_norm(power_(state), self.true_signal_power)
        score = self.similarity_l2_norm(power_(state), self.true_signal_power)
        return score
