...
def S(i, j, func, matrix_array, x_opt, parameter_i, N_parameters, mu, sigma, x, r):
    ...
    if j==1:
        ...
        base = UDR_evCalculation(j, func, i, x_opt, parameter_i, N_parameters, mu, sigma, x, r, matrix_array)
        return base
        ...
        for k in np.arange(i+1): # Sum k=0 up to i
            ...
            parameter_sigma += binom(i, k) * UDR_evCalculation(j, func, i - k, x_opt, parameter_i, N_parameters, mu, sigma, x, r, matrix_array) * S_val
        return parameter_sigma


def compute_moment_matrices(N_parameters, n):
    """
    ...
    for j in np.arange(1, N_parameters + 1): # j=1,..,N
        error_distribution = normal_pdf # use a standard normal and do a co-ordinate transform later
...
def UDR_evCalculation(j, func, l, x_opt, parameter_i, N_parameters, mu, sigma, x, r, matrix_array):
    """
    ...
    for i in np.arange(1, n+1): # sum i = 1 ... n
        ...
        x_perturb[parameter_i] += val_sample_point * sigma + mu
...
def UDR_moments(func, l, x_opt, parameter_i, N_parameters, mu, sigma, x, r, matrix_array):
    ...
    for i in np.arange(l+1): # sum i = 0 to l
        ...
        moment += binom(l, i)*sval*(-(N_parameters-1)*0)**(l-i)     #func(mu) = 0, due to shifting in compute_with_error

    return moment


def UDRAnalysis_test(x_opt, func, mu_lst, sigma_lst, m_num_moments=5):
    """
    """
    ...
    for parameter_i, (mu, sigma) in enumerate(zip(mu_lst, sigma_lst)):
        matrix_moments = compute_moment_matrices(N_parameters, 5)
