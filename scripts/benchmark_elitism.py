...
def benchmark_elitism(population_sizes, elitism_ratios, start_graph, evaluator, ga_opts, io, update_rule='tournament'):
    hof_size = ga_opts['n_hof']
    top_level_path = io.path
    repetitions = 10

    for pop_size in population_sizes:
        for elitism_ratio in elitism_ratios:
            io.path = top_level_path
            io.init_save_dir(sub_path=f"popSize{pop_size}_elitismRatio{str(elitism_ratio).replace('.', '_')}", unique_id=False)
            ...
            for i in range(repetitions):
                ...
                hof, log = topology_optimization(copy.deepcopy(start_graph), propagator, evaluator, evolver, io,
                                                ...
                                                parameter_opt_method='L-BFGS+GA', elitism_ratio=elitism_ratio,
...
if __name__ == '__main__':
    ...
    io.init_save_dir(sub_path=f'benchmark_elitism_{options_cl.evaluator}', unique_id=True)
    ...
    benchmark_elitism([8, 16], [0, 0.1, 0.3], start_graph, evaluator, gen_ga_opts, io, update_rule=options_cl.selection)