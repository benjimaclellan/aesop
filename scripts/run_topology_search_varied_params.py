...
if __name__ == '__main__':
    ...
    ga_opts = {'n_generations': 10,
               'n_population': 9, # psutil.cpu_count(),
               'n_hof': 1,
               'verbose': True,
    ...
    # evolver = SizeAwareLookupEvolver(verbose=False, debug=False)
    ...
    update_rules = ['tournament', 'roulette']

    for rule in update_rules:
        for evolver in evolvers:
            print(f'Starting optimization with rule {rule}, evolver: {evolver.__class__.__name__}')
            io = handle_io()
            hof, log = topology_optimization(copy.deepcopy(start_graph), propagator, evaluator, evolver, io, ga_opts=ga_opts, local_mode=True, update_rule=rule, crossover_maker=None, parameter_opt_method='NULL')
